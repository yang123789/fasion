...
Page({
  ...
  onShow: function () {
    ...
    if (order_tag == 'pintuan') {
      ...
      parm.order_tuangou_group_id = group_id;
    ...
    app.sendRequest({
      url: 'api.php?s=Pintuan/paymentOrder',
      data: parm,
      success: function (res) {
        ...
        if (code == 0) {
          ...
          let new_pay_money = parseFloat(data.count_money) + parseFloat(data.express) - balance;

          //未开启商家配送 开启买家自提
          if (shop_config.seller_dispatching == 0 && shop_config.buyer_self_lifting == 1) {
            new_pay_money = parseFloat(data.count_money) + parseFloat(data.pick_up_money) - balance;
          ...
          shop_config.order_designated_delivery_time = 0;
          let order_info = data;
          //图片处理
  ...
  selectPayType: function (e) {
    ...
    let pay_money = count_money + express - balance;
  ...
  selectExpressCompany: function (e) {
    ...
    let pay_money = count_money + express_money - balance;
    let old_pay_money = goods_count_money + express_money;
    let order_invoice_tax = parseFloat(that.data.shop_config.order_invoice_tax); //发票税率
    let invoice_need = that.data.invoice_need; //是否需要发票
    let order_invoice_money = invoice_need == 1 ? order_invoice_tax / 100 * old_pay_money : 0.00;
    pay_money = parseFloat(order_invoice_money) + parseFloat(pay_money);
  ...
  inputPoint: function (event) {
    ...
    let order_invoice_tax = parseFloat(that.data.shop_config.order_invoice_tax); //发票税率
    let invoice_need = that.data.invoice_need; //是否需要发票
    let pay_money = count_money + express;
    let old_pay_money = goods_count_money + express;
    let order_invoice_money = invoice_need == 1 ? order_invoice_tax / 100 * old_pay_money : 0.00;
  ...
  commitOrder: function (event) {
    ...
    app.sendRequest({
      url: 'api.php?s=Pintuan/pintuanOrderCreate',
      data: {
        tuangou_group_id: group_id,
        goods_sku_list: goods_sku_list,
        buyer_message: leavemessage,
      ...
      success: function (res) {
        ...
        if (code == 0) {
          ...
          if (pay_type == 5 || pay_money == 0) {
            ...
            if (order_tag == 'pintuan' && order_goods_type == 1) {
              app.setTabType(3);
            } else {
              app.setTabType('');
            }
