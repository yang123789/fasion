...
Page({
  ...
  onShow: function () {
    ...
    app.restStatus(that, 'customerFlag');
  ...
  customer: function (event) {
    let that = this;
    let customerFlag = that.data.customerFlag;
    let status = that.data.status;
    let order_goods_id = event.currentTarget.dataset.id;
    ...
    app.clicked(that, 'customerFlag');

    wx.navigateTo({
      url: '/pagesother/pages/order/customerdetail/customerdetail?id=' + order_goods_id + '&status=' + status,
    })
  },
